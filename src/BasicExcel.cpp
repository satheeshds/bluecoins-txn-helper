#include "../include/BasicExcel.hpp"
#include <string.h>
...
namespace YCompoundFiles
{
    ...
    Block::Block() : blockSize_(512), fileSize_(0), indexEnd_(0),
                     filename_(0) {}
    ...
    // REQUIRE: Block of data must be at least 512 bytes in size.
    {
        ...
        for (size_t i = 0; i < 109; ++i)
            LittleEndian::Write(block, BATArray_[i], 0x004C + i * 4, 4);
    }
    ...
    CompoundFile::Property::Property() : nameSize_(0),
    ...
    CompoundFile::CompoundFile() : block_(512), properties_(0), propertyTrees_(0),
    ...
    // PROMISE: Return true if file header contain magic number, false if otherwise.
    {
        ...
        if (header_.fileType_ != 0xE11AB1A1E011CFD0LL)
            return false;
    ...
    // PROMISE: Returns the startIndex of new data for the property.
    {
        if (isBig)
        {
            ...
            if (indices.empty())
                ...
                index = (startIndex != -2) ? indices[curIndex - 1] : 0;
        ...
        else
        {
            ...
            if (extraBlocks > 0)
            {
                ...
                // Place new end marker
                if (maxNewBlocks != 0)
                    sblocksIndices_[indices[maxNewBlocks] - 1] = -2;
                else
                    startIndex = -2;
    ...
    // EXPLAIN: isBig is true if property uses big blocks, false if it uses small blocks.
    {
        ...
        if (isBig)
        {
            for (size_t i = startIndex; i != -2; i = blocksIndices_[i])
                indices.push_back(i);
        }
        else
        {
            for (size_t i = startIndex; i != -2; i = sblocksIndices_[i])
                indices.push_back(i);
        }
    }
                ...
                for (size_t i = 0; i < maxIndices; ++i)
                {
                    for (size_t j = 0; j < maxSBATindices; ++j)
                    {
                        ...
                        if (sblocksIndices_[j] == indices[i])
                            sblocksIndices_[j] = sblocksIndices_[indices[i]];
                        if (sblocksIndices_[j] > indices[i] &&
                            sblocksIndices_[j] != -1 &&
                            sblocksIndices_[j] != -2)
                            --sblocksIndices_[j];
    ...
    {
        if (previousDirectories_.empty())
            previousDirectories_.push_back(propertyTrees_);
    ...
    // PROMISE: will be readjusted to accomodate the next property.
    {
        ...
        {
            if (index < parentTree->children_[i]->index_)
                break;
        }
    ...
    // PROMISE: and next property will be readjusted to accomodate the deleted property.
    {
        ...
        // Remove property
        if (properties_[tree->index_])
            delete properties_[tree->index_];
...
namespace YExcel
{
    ...
    SmallString::SmallString(const SmallString &s) : name_(0), wname_(0), unicode_(s.unicode_)
    {
        if (s.name_)
        {
    ...
    LargeString::LargeString(const LargeString &s) : name_(s.name_), wname_(s.wname_),
    ...
    {
        if (unicode_ & 1)
            return wname_.size() * SIZEOFWCHAR_T;
        else
            return name_.size();
    }
    /************************************************************************************************************/
    ...
    Worksheet::CellTable::RowBlock::CellBlock::MulRK::XFRK::XFRK() : XFRecordIndex_(0), RKValue_(0){};
    ...
    /************************************************************************************************************/
    Worksheet::CellTable::RowBlock::CellBlock::CellBlock() : type_(-1), normalType_(true){};
    ...
    size_t Worksheet::CellTable::RowBlock::Read(const char *data)
    {
        ...
        while (code != CODE::DBCELL)
        {
            ...
            case CODE::FORMULA:
                ...
                if (cellBlocks_.size() % 1000 == 0)
                    cellBlocks_.reserve(cellBlocks_.size() + 1000);
    ...
    bool BasicExcel::SaveAs(const char *filename)
    {
        ...
        if (!file_.Create(filename))
            return false;
        if (file_.MakeFile("Workbook") != CompoundFile::SUCCESS)
            return false;
    ...
    BasicExcelWorksheet *BasicExcel::GetWorksheet(const char *name)
    {
        ...
        for (size_t i = 0; i < maxWorksheets; ++i)
        {
            if (workbook_.boundSheets_[i].name_.unicode_ & 1)
            {
                ...
                if (wcsncmp(&wsName[0], workbook_.boundSheets_[i].name_.wname_, wcslen(&wsName[0])) == 0)
                    return &(yesheets_[i]);
            }
            else
            {
    ...
    BasicExcelWorksheet *BasicExcel::GetWorksheet(const wchar_t *name)
    {
        ...
        for (size_t i = 0; i < maxWorksheets; ++i)
        {
            if (!(workbook_.boundSheets_[i].name_.unicode_ & 1))
            {
                ...
                if (strncmp(&sName[0], workbook_.boundSheets_[i].name_.name_, strlen(&sName[0])) == 0)
                    return &(yesheets_[i]);
            }
            else
            {
    ...
    bool BasicExcel::RenameWorksheet(size_t sheetIndex, const wchar_t *to)
    {
        ...
        if (sheetIndex < maxWorksheets)
        {
            for (size_t i = 0; i < maxWorksheets; ++i)
            {
                if (!(workbook_.boundSheets_[i].name_.unicode_ & 1))
                    continue;
                if (wcscmp(to, workbook_.boundSheets_[i].name_.wname_) == 0)
                    return false;
            }
            workbook_.boundSheets_[sheetIndex].name_ = to;
            return true;
    ...
    void BasicExcel::UpdateWorksheets()
    {
        ...
        for (size_t s = 0; s < maxWorksheets; ++s)
        {
            ...
            if (s > 0)
                worksheets_[s].window2_.options_ &= ~0x200;
            ...
            for (size_t r = 0, curRowBlock = 0; r < maxRows; ++r)
            {
                ...
                for (size_t c = 0; c < maxCols; ++c)
                {
                    ...
                    if (cellType != BasicExcelCell::UNDEFINED) // Current cell contains some data
                    {
                        ...
                        if (pCellBlocks->size() % 1000 == 0)
                            pCellBlocks->reserve(pCellBlocks->size() + 1000);
                        ...
                        case BasicExcelCell::STRING:
                        {
                            ...
                            if (stringMapIt != stringMap.end())
                                strIndex = stringMapIt->second;
                            else
                                strIndex = maxUniqueStrings;
                        ...
                        case BasicExcelCell::WSTRING:
                        {
                            ...
                            if (wstringMapIt != wstringMap.end())
                                strIndex = wstringMapIt->second;
                            else
                                strIndex = maxUniqueStrings;
    ...
    /************************************************************************************************************/
    BasicExcelWorksheet::BasicExcelWorksheet(BasicExcel *excel, size_t sheetIndex) : excel_(excel), sheetIndex_(sheetIndex)
    {
        UpdateCells();
    }
    ...
    bool BasicExcelWorksheet::Rename(const char *to)
    {
        ...
        for (size_t i = 0; i < maxWorksheets; ++i)
        {
            if (excel_->workbook_.boundSheets_[i].name_.unicode_ & 1)
                continue;
            if (strcmp(to, excel_->workbook_.boundSheets_[i].name_.name_) == 0)
                return false;
        }
    ...
    bool BasicExcelWorksheet::Rename(const wchar_t *to)
    {
        ...
        for (size_t i = 0; i < maxWorksheets; ++i)
        {
            if (!(excel_->workbook_.boundSheets_[i].name_.unicode_ & 1))
                continue;
            if (wcscmp(to, excel_->workbook_.boundSheets_[i].name_.wname_) == 0)
                return false;
        }
